...
export default function ListCategoria() {
    ...
    function Delete(idpubli) {
        ...
        try {
            const getList = async () => {
                const res = await fetch('/api/carrusel/' + idpubli, {
                    method: 'DELETE',
                })
                const data = await res.json()
            }
    ...
    return (
        <div class="relative overflow-x-auto">
            <h1 class="flex items-center text-5xl font-extrabold dark:text-white">Lista de Categorias</h1>
            <br></br>
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3">Arte</th>
                        <th scope="col" class="px-6 py-3">Creado</th>
                        <th scope="col" class="px-6 py-3">Finaliza</th>
                ...
                <tbody>
                    {Lista.map((tasks) => (
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{tasks.titulo}</td>
                            <td class="px-6 py-4"><Image src={tasks.arte} width={130} height={130} /></td>
                            <td id='linkExten' scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{format(tasks.createIn, 'dd/MM/yyyy')}</td>
                            <td id='linkExten' scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{format(tasks.finalpublic, 'dd/MM/yyyy')}</td>
