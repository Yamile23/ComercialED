...
export default function ListCategoria() {
    ...
    function Delete(idpubli) {
        ...
        try {
            const getList = async () => {
                const res = await fetch('/api/tasks/' + idpubli, {
                    method: 'DELETE',
                })
                const data = await res.json()
            }
                ...
                <tbody>
                    {Lista.map((tasks) => (
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <td id='linkExten' scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{tasks.detalle}</td>
