...
export default function ListPublic() {
  ...
  function Delete(idpubli) {
    ...
    try{
      const getList = async () => {
        const res = await fetch('/api/publicaciones/'+ idpubli, {
          method: 'DELETE',
        })
        const data = await res.json()
      }
  ...
  return (
    <div class="relative overflow-x-auto">
            <h1 class="flex items-center text-5xl font-extrabold dark:text-white">Lista de Publicaciones</h1>
            <br></br>
          ...
          <th scope="col" class="px-6 py-3">Enlace</th>
          <th scope="col" class="px-6 py-3">Arte</th>
          <th scope="col" class="px-6 py-3">Creacion</th>
          <th scope="col" class="px-6 py-3">Finalizacion</th>
        ...
        <tbody>
          {Lista.map((tasks) => (
            ...
            <td class="px-6 py-4">{tasks.id}</td>
            <td class="px-6 py-4">{tasks.titulo}</td>
            <td class="px-6 py-4">{tasks.detalle}</td>
            <td id='linkExten' class="px-6 py-4">{tasks.link}</td>
            <td class="px-6 py-4"><Image src={"/img/"+tasks.arte} width={230} height={230} /></td>
            <td class="px-6 py-4">{format(tasks.createIn, 'dd/MM/yyyy')}</td>
            <td class="px-6 py-4">{format(tasks.finalpublic, 'dd/MM/yyyy')}</td>
